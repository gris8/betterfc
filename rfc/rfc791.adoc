...
been called a "catenet" <<ref1>>.  The internet protocol provides for
...
local net module which would add the 1822 leader <<ref2>> to the internet
...
Protocol (ICMP) <<ref3>> which is implemented in the internet protocol
module.
...
A distinction is made between names, addresses, and routes <<ref4>>.
A name indicates what we seek.  An address indicates where it is.
A route indicates how to get there.  The internet protocol deals
...
addresses.
Examples of address mappings may be found in "Address Mappings" <<ref5>>.
...
circumstances.  If [.rfc-error-text]#internet datagram# marked don't fragment cannot be
delivered to its destination without fragmenting it, it is to be
discarded instead.

Fragmentation, transmission and reassembly across a local network
which is invisible to the internet protocol module is called
intranet fragmentation and may be used <<ref6>>.
...
(GGP) <<ref7>> to coordinate routing and other internet control
information.
...
Bits   5 :: 0 = Normal [.rfc-error-text]#Relibility#, 1 = High [.rfc-error-text]#Relibility#.
...
is given in "Service Mappings" <<ref8>>.
...
Total Length is the length of [.rfc-error-text]#the datagram#, measured in octets,
...
are specified in "Assigned Numbers" <<ref9>>.
...
+
The source address.  See section <<_discussion>>.

Destination Address:  32 bits::
+
The destination address.  See section <<_discussion>>.
...
[[errata_5038]]
[.rfc-error-text]#The option begins with the option type code.  The second octet
...
size of the option does not change due to adding addresses. [[errata_579]] [.rfc-error-text]#The
...
datagram is being forwarded into the recorded route [[errata_2294]] [.rfc-error-text]#begining# at
...
[[errata_4752]]
[.rfc-error-text]#The Option Length is the number of octets in the option counting
...
timestamps.  [[errata_583]] [.rfc-error-text]#The intitial contents of the timestamp data area
...
This format allows 2{dblstar_}13 = 8192 fragments of 8 octets each for a
total of 65,536 octets.  Note that this is consistent with [[errata_578]] [.rfc-error-text]#the the#
...
a fragmentation procedure that [[errata_2295]] [.rfc-error-text]#repeatly# divided large datagrams in
...
mappings in "Service Mappings" [<<ref8,8>>].
...
Internet protocol errors may be reported via the ICMP messages [<<ref3,3>>].
...
[[errata_5037]]
NFB::
          [.rfc-error-text]#The Number of Fragment Blocks in a the data portion of an
...
- [[[ref1,1]]] Cerf, V., "The Catenet Model for Internetworking," Information
               Processing Techniques Office, Defense Advanced Research Projects
               Agency, IEN 48, July 1978.

- [[[ref2,2]]] Bolt Beranek and Newman, "Specification for the Interconnection of
               a Host and an IMP," BBN Technical Report 1822, Revised May 1978.

- [[[ref3,3]]] Postel, J., "Internet Control Message Protocol - DARPA Internet
               Program Protocol Specification," RFC 792, USC/Information Sciences
               Institute, September 1981.

- [[[ref4,4]]] Shoch, J., "Inter-Network Naming, Addressing, and Routing,"
               COMPCON, IEEE Computer Society, Fall 1978.

- [[[ref5,5]]] Postel, J., "Address Mappings," RFC 796, USC/Information Sciences
               Institute, September 1981.

- [[[ref6,6]]] Shoch, J., "Packet Fragmentation in Inter-Network Protocols,"
               Computer Networks, v. 3, n. 1, February 1979.

- [[[ref7,7]]] Strazisar, V., "How to Build a Gateway", IEN 109, Bolt Beranek and
               Newman, August 1979.

- [[[ref8,8]]] Postel, J., "Service Mappings," RFC 795, USC/Information Sciences
               Institute, September 1981.

- [[[ref9,9]]] Postel, J., "Assigned Numbers," RFC 790, USC/Information Sciences
               Institute, September 1981.
...
// don't show the example number
//:example-number!:
//:figure-number!:

== Errata

For an explanation of the different error categories please visit the link https://www.rfc-editor.org/errata-definitions .
...
////
Asciidoctor: how to use Passthrough Macros: https://asciidoctor.org/docs/user-manual/#pass-macros
////
...
<<errata_716,It should say>> :

[subs=+macros]
....

        +--------+--------+--------+--------+
        |10001000|00000pass:quotes[*100*]|  Stream ID  |
        +--------+--------+--------+--------+
         Type=136 Length=4
...
[subs=+macros]
....
The pass:quotes[*intitial*] contents of the route data area must be zero.
....

<<errata_579,It should say>> :

[subs=+macros]
....
The pass:quotes[*initial*] contents of the route data area must be zero.
....
...
[subs=+macros]
....
The pass:quotes[*intitial*] contents of the timestamp data area must be zero
or internet address/zero pairs.
....

<<errata_583,It should say>> :

[subs=+macros]
....
The pass:quotes[*initial*] contents of the timestamp data area must be zero
or internet address/zero pairs.
....
...
....
NFB
          The Number of Fragment Blocks in pass:quotes[*a*] the data portion of an
          internet fragment.  That is, the length of a portion of data
          measured in 8 octet units.
....

<<errata_5037,It should say>> :

[subs=+macros]
....
NFB
          The Number of Fragment Blocks in pass:quotes[*the*] data portion of an
          internet fragment.  That is, the length of a portion of data
          measured in 8pass:quotes[*-*]octet units.
....
...
....
The option begins with the option type code.  The second octet
        is the option length pass:quotes[*which includes the option type code and*] the
        length octet, the pointer octet, and length-3 octets of route
        data.
....

<<errata_5038,It should say>> :

[subs=+macros]
....
The option begins with the option type code.  The second octet
        is the option length, pass:quotes[*measured in octets, including the option
...
pointer, pass:quotes[*and overflow/flag octets*] (maximum length 40).
....

<<errata_4752,It should say>>:

[subs=+macros]
....
The Option Length is the number of octets
in the option counting the type, length,
pointer, pass:quotes[*overflow/flag octets
...
[subs=+macros]
....
Note that this is consistent with pass:quotes[*the the*] datagram total length field (of
course, the header is counted in the total length and not in the
fragments).
....

<<errata_578,It should say>>:

[subs=+macros]
....
Note that this is consistent with pass:quotes[*the*] datagram total length field (of
course, the header is counted in the total length and not in the
fragments).
....
...
a fragmentation procedure that pass:quotes[*repeatly*] divided large datagrams in
half until the resulting fragments were less than the maximum
transmission unit size.
....

<<errata_2295,It should say>>:

[subs=+macros]
....
For example, one could implement
a fragmentation procedure that pass:quotes[*repeatedly*] divided large datagrams in
half until the resulting fragments were less than the maximum
transmission unit size.
....
...
own internet address as known in the environment into which this
datagram is being forwarded into the recorded route pass:quotes[*begining*] at
the octet indicated by the pointer, and increments the pointer
by four.
....

<<errata_2294,It should say>>:

[subs=+macros]
....
If it is, it inserts its
own internet address as known in the environment into which this
datagram is being forwarded into the recorded route pass:quotes[*beginning*] at
the octet indicated by the pointer, and increments the pointer
by four.
....
...
[subs=+macros]
....
Total Length is the length of pass:quotes[*the datagram*], measured in octets,
including internet header and data.
....

<<errata_2519,It should say>>:

[subs=+macros]
....
Total Length is the length of pass:quotes[*the datagram or fragment*], measured in octets,
including internet header and data.
....
...
[subs=+macros]
....
Bits   5:  0 = Normal pass:quotes[*Relibility*], 1 = High pass:quotes[*Relibility*].
....

<<errata_3074,It should say>>:

[subs=+macros]
....
Bits   5:  0 = Normal pass:quotes[*Reliability*], 1 = High pass:quotes[*Reliability*].
....
...
[subs=+macros]
....
    If pass:quotes[*internet datagram*] marked don't fragment cannot be
    delivered to its destination without fragmenting it, it is to be
    discarded instead.
....

<<errata_4126,It should say>>:

[subs=+macros]
....
    If pass:quotes[*an internet datagram*] marked don't fragment cannot be
    delivered to its destination without fragmenting it, it is to be
    discarded instead.
....
