...
[frame=none, cols="<,>", width="99%", stripes=none]
|===
...
// it enables the section numbering
:numbered:
...
Discovery [<<ref_RFC6763>>] were created to provide IP networking with the
ease-of-use and autoconfiguration for which AppleTalk was well-known
[<<ref_RFC6760>>].  When reading this document, familiarity with the concepts
of Zero Configuration Networking [<<ref_Zeroconf>>] and automatic link-local
addressing [<<ref_RFC3927>>] [<<ref_RFC4862>>] is helpful.

Multicast DNS borrows heavily from the existing DNS protocol
[<<ref_RFC1034>>] [<<ref_RFC1035>>] [<<ref_RFC6195>>], using the existing DNS message
...
RFCs to Indicate Requirement Levels" [<<ref_RFC2119>>].
...
port 5353 is discussed in <<appendix_A>>.
...
resource record sets [<<ref_RFC1034>>]:
...
in Section <<_probing>>, "Probing".  (For fault-tolerance and other
...
addresses using a hash function is discussed in <<appendix_B>>.
...
configured it, or because it was received via DHCP [<<ref_RFC2132>>] or via
...
important to use DNS Security Extensions (DNSSEC) [<<ref_RFC4033>>] or other
...
as those used by DNS-Based Service Discovery [<<ref_RFC6763>>]) may contain
...
issues are discussed further in <<appendix_C>>.
...
the reserved Multicast DNS domains (see Sections <<_multicast_dns_names>> and <<_reverse_address_mapping>>), then,
...
with a list of DNS-SD services found on the network [<<ref_RFC6763>>], then
...
in Section <<_known_answer_suppression>>.  The Known-Answer Suppression mechanism tells
...
<<_announcements_to_flush_outdated_cache_entries>>, "Announcements to Flush Outdated Cache Entries").  In this
...
When a question is retransmitted (as described in Section <<_continuous_multicast_dns_querying>>), the
...
querier should have a large Known-Answer list (Section <<_known_answer_suppression>>) so that
...
<<appendix_D>>).  In addition, the unicast-response bit SHOULD be set
...
case of answering a probe question (Section <<_probing>>) with the unicast-
...
as multicast replies, including the cache-flush bit (Section <<_announcements_to_flush_outdated_cache_entries>>)
and (except when defending a unique name against a probe from another
host) randomized delays to reduce network collisions (Section <<_responding>>).
...
questions, as described above in Section <<_questions_requesting_unicast_responses>>.  Since it is possible
...
<<_negative_responses>>, "Negative Responses".  For shared records, which are owned by no
...
A Multicast DNS responder on Ethernet [<<ref_IEEE.802.3>>] and similar shared
...
Section <<_simultaneous_probe_tiebreaking>>, "Simultaneous Probe Tiebreaking").
...
Service Discovery [<<ref_RFC6763>>], where a large number of responses may be
anticipated.
...
described in Section <<_multipacket_known_answer_suppression>>, "Multipacket Known-Answer Suppression".
...
* by virtue of being a legacy query (Section <<_legacy_unicast_responses>>), or
...
mechanisms described in Section <<_passive_observation_of_failures_poof>> would not work correctly.  Other
benefits of sending responses via multicast are discussed in
<<appendix_D>>.  A Multicast DNS querier MUST only accept unicast responses if
...
[<<ref_RFC4034>>].  In the case of Multicast DNS the NSEC record is not being
used for its usual DNSSEC [<<ref_RFC4033>>] security properties, but simply
...
of the DNS NSEC record [<<ref_RFC4034>>], including bitmaps up to 65,536 bits
...
* [.rfc-error-text]#The 'Next Domain Name' field contains the record's own name.
When used with name compression, this means that the 'Next
Domain Name' field always takes exactly two bytes in the
message.#

* The Type Bit Map block number is 0.

* The Type Bit Map block length byte is a value in the range 1-32.

* The Type Bit Map data is 1-32 bytes, as indicated by length
byte.
...
is typically 120 seconds (see Section <<_resource_record_ttl_values_and_cache_coherency>>), so the negative cache
...
responses is discussed further in <<appendix_E>>.
...
// sec.6.3
=== Responding to Multiquestion Queries
...
// sec.6.5
=== Wildcard Queries (qtype "ANY" and qclass "ANY")
...
// sec.6.6
=== Cooperating Multicast DNS Responders
...
* If A's resource record is intended to be a shared resource
record, then this is no conflict, and no action is required.

* If A's resource record is intended to be a member of a unique
resource record set owned solely by that responder, then this is
a conflict and MUST be handled as described in Section <<_conflict_resolution>>,
"Conflict Resolution".
...
* If the TTL of B's resource record given in the message is at
least half the true TTL from A's point of view, then no action
is required.

* If the TTL of B's resource record given in the message is less
...
simple resolver such as described in Section <<_one_shot_multicast_dns_queries>>, "One-Shot Multicast
...
bit described in Section <<_announcements_to_flush_outdated_cache_entries>>, "Announcements to Flush Outdated Cache
...
coherency mechanisms described in Section <<_resource_record_ttl_values_and_cache_coherency>>, "Resource Record TTL
...
// sec.7.2
=== Multipacket Known-Answer Suppression
...
steps below: Probing (Section <<_probing>>) and Announcing (Section <<_announcing>>).
...
[<<ref_RFC6763>>] in the same query message.
...
<<_simultaneous_probe_tiebreaking>>, "Simultaneous Probe Tiebreaking", below).  In the case of a host
...
Update message [<<ref_RFC2136>>] [<<ref_RFC3007>>].
...
in Section <<_announcements_to_flush_outdated_cache_entries>>), then the record type, then raw comparison of the
...
name compression [<<ref_RFC1035>>], the names MUST be uncompressed before
...
// sec.8.2.1
==== Simultaneous Probe Tiebreaking for Multiple Records
...
Based Service Discovery [<<ref_RFC6763>>]), the records are simply placed as
...
and is discussed in more detail below in Section <<_announcements_to_flush_outdated_cache_entries>>, "Announcements
...
record, the conflict MUST be resolved as described in Section<<_conflict_resolution>>,
"Conflict Resolution".
...
announcement with RR TTL zero (see Section <<_goodbye_packets>>, "Goodbye Packets")
...
Section <<_probing_and_announcing_on_startup>>, "Probing and Announcing on Startup".  The protocol used in
...
// sec.10
== Resource Record TTL Values and Cache Coherency
...
Section <<_cooperating_multicast_dns_responders>> above, if one of the responders shuts down and
...
// sec.10.2
=== Announcements to Flush Outdated Cache Entries
...
records, as described above in Section <<_probing>>, "Probing".
...
Section <<_announcing>>, "Announcing", and to responses generated as a result of
...
(see Section <<_questions_requesting_unicast_responses>>, "Questions Requesting Unicast Responses").
...
to pseudo-RRs like OPT [<<ref_RFC2671>>], TSIG [<<ref_RFC2845>>], TKEY [<<ref_RFC2930>>],
SIG0 [<<ref_RFC2931>>], etc., that pertain only to a particular transport
...
// sec.10.3
=== Cache Flush on Topology change
...
response is preferred (see Section <<_questions_requesting_unicast_responses>>, "Questions Requesting Unicast
...
// sec.12
== Special Characteristics of Multicast DNS Domains
...
// sec.13
== Enabling and Disabling Multicast DNS
...
// sec.14
== Considerations for Multiple Interfaces
...
connection and an 802.11 [<<ref_IEEE.802.11>>] wireless connection active at
...
Section <<_announcements_to_flush_outdated_cache_entries>>.)


// sec.15
== Considerations for Multiple Responders on the Same Machine
...
// sec.15.2
=== Multipacket Known-Answer lists
...
DNS Specification" [<<ref_RFC2181>>] directly discusses the subject of
...
names to be just letters, digits, and hyphens [<<ref_RFC1034>>], and since
...
DNS MUST be encoded as precomposed UTF-8 [<<ref_RFC3629>>] "Net-Unicode"
[<<ref_RFC5198>>] text.
...
For names that are restricted to US-ASCII [<<ref_RFC0020>>] letters, digits,
...
[<<ref_RFC3492>>] are discussed further in <<appendix_F>>.

The simple rules for case-insensitivity in Unicast DNS [<<ref_RFC1034>>]
[<<ref_RFC1035>>] also apply in Multicast DNS; that is to say, in name
...
// sec.17
== Multicast DNS Message Size

The 1987 DNS specification [<<ref_RFC1035>>] restricts DNS messages carried
...
"Jumbo" packet [<<ref_Jumbo>>].  However, in practice Ethernet "Jumbo"
...
In both multicast query and multicast response messages, the
Authentic Data bit [<<ref_RFC2535>>] MUST be zero on transmission, and MUST
...
Disabled bit [<<ref_RFC2535>>] MUST be zero on transmission, and MUST be
ignored on reception.
...
for this particular question.  (See Section <<_questions_requesting_unicast_responses>>.)


// sec.18.13
=== Repurposing of Top Bit of rrclass in Resource Record Sections
...
records to fit in a single packet).  (See Section <<_announcements_to_flush_outdated_cache_entries>>.)
...
earlier in the message [<<ref_RFC1035>>].
...
Until future IETF Standards Action [<<ref_RFC5226>>] specifying that names in
...
is possible that no future IETF Standards Action [<<ref_RFC5226>>] will be
...
specification in 1996 [<<ref_RFC2052>>] may not decode these compressed
...
// sec.19
== Summary of Differences between Multicast DNS and Unicast DNS
...
IPsec signatures and/or DNSSEC [<<ref_RFC4033>>] signatures so that they can
...
"Ink-Jet._pdl-datastream._tcp.local.") are special [<<ref_RFC6761>>] in the
following ways:
...
[horizontal]
[[ref_MC4,MC4]] [MC4] :: IANA, "IPv4 Multicast Address Space Registry",
           <http://www.iana.org/assignments/multicast-addresses/>.

[[ref_MC6,MC6]] [MC6] :: IANA, "IPv6 Multicast Address Space Registry",
           <http://www.iana.org/assignments/
           ipv6-multicast-addresses/>.

[[ref_RFC0020,RFC0020]] [RFC0020] :: Cerf, V., "ASCII format for network interchange", RFC 20,
           October 1969.

[[ref_RFC1034,RFC1034]] [RFC1034] :: Mockapetris, P., "Domain names - concepts and facilities",
           STD 13, RFC 1034, November 1987.

[[ref_RFC1035,RFC1035]] [RFC1035] :: Mockapetris, P., "Domain names - implementation and
           specification", STD 13, RFC 1035, November 1987.

[[ref_RFC2119,RFC2119]] [RFC2119] :: Bradner, S., "Key words for use in RFCs to Indicate
           Requirement Levels", BCP 14, RFC 2119, March 1997.

[[ref_RFC3629,RFC3629]] [RFC3629] :: Yergeau, F., "UTF-8, a transformation format of ISO
           10646", STD 63, RFC 3629, November 2003.

[[ref_RFC4034,RFC4034]] [RFC4034] :: Arends, R., Austein, R., Larson, M., Massey, D., and S.
           Rose, "Resource Records for the DNS Security Extensions",
           RFC 4034, March 2005.

[[ref_RFC5198,RFC5198]] [RFC5198] :: Klensin, J. and M. Padlipsky, "Unicode Format for Network
           Interchange", RFC 5198, March 2008.

[[ref_RFC6195,RFC6195]] [RFC6195] :: Eastlake 3rd, D., "Domain Name System (DNS) IANA
           Considerations", BCP 42, RFC 6195, March 2011.

[[ref_RFC6761,RFC6761]] [RFC6761] :: Cheshire, S. and M. Krochmal, "Special-Use Domain Names",
           RFC 6761, February 2013.

[[ref_SN,SN]] [SN] ::      IANA, "Service Name and Transport Protocol Port Number
...
[horizontal]
[[ref_B4W,B4W]]   [B4W]  ::    "Bonjour for Windows",
              <http://en.wikipedia.org/wiki/Bonjour_(software)>.

 [[ref_BJ,BJ]]  [BJ] ::      Apple Bonjour Open Source Software,
              <http://developer.apple.com/bonjour/>.

[[ref_IEEE.802.3,IEEE.802.3]]   [IEEE.802.3] ::
...
[[ref_IEEE.802.11,IEEE.802.11]]   [IEEE.802.11] ::
              "Information technology - Telecommunications and
              information exchange between systems - Local and
              metropolitan area networks - Specific requirements - Part
...
[[ref_Jumbo,Jumbo]]   [Jumbo] ::   "Ethernet Jumbo Frames", November 2009,
              <http://www.ethernetalliance.org/library/whitepaper/
              ethernet-jumbo-frames/>.

[[ref_NIAS,NIAS]]   [NIAS]  ::   Cheshire, S. "Discovering Named Instances of Abstract
              Services using DNS", Work in Progress, July 2001.

[[ref_NSD,NSD]]   [NSD]  ::    "NsdManager | Android Developer", June 2012,
              <http://developer.android.com/reference/
              android/net/nsd/NsdManager.html>.

 [[ref_RFC2052,RFC2052]]  [RFC2052] :: Gulbrandsen, A. and P. Vixie, "A DNS RR for specifying the
              location of services (DNS SRV)", RFC 2052, October 1996.

[[ref_RFC2132,RFC2132]]   [RFC2132] :: Alexander, S. and R. Droms, "DHCP Options and BOOTP Vendor
              Extensions", RFC 2132, March 1997.

[[ref_RFC2136,RFC2136]]   [RFC2136] :: Vixie, P., Ed., Thomson, S., Rekhter, Y., and J. Bound,
              "Dynamic Updates in the Domain Name System (DNS UPDATE)",
              RFC 2136, April 1997.

[[ref_RFC2181,RFC2181]]   [RFC2181] :: Elz, R. and R. Bush, "Clarifications to the DNS
              Specification", RFC 2181, July 1997.

[[ref_RFC2535,RFC2535]]   [RFC2535] :: Eastlake 3rd, D., "Domain Name System Security
              Extensions", RFC 2535, March 1999.

[[ref_RFC2671,RFC2671]]   [RFC2671] :: Vixie, P., "Extension Mechanisms for DNS (EDNS0)", RFC
              2671, August 1999.

[[ref_RFC2845,RFC2845]]   [RFC2845] :: Vixie, P., Gudmundsson, O., Eastlake 3rd, D., and B.
              Wellington, "Secret Key Transaction Authentication for DNS
              (TSIG)", RFC 2845, May 2000.

[[ref_RFC2930,RFC2930]]   [RFC2930] :: Eastlake 3rd, D., "Secret Key Establishment for DNS (TKEY
              RR)", RFC 2930, September 2000.

[[ref_RFC2931,RFC2931]]   [RFC2931] :: Eastlake 3rd, D., "DNS Request and Transaction Signatures
              ( SIG(0)s )", RFC 2931, September 2000.

[[ref_RFC3007,RFC3007]]   [RFC3007] :: Wellington, B., "Secure Domain Name System (DNS) Dynamic
              Update", RFC 3007, November 2000.

[[refRFC3492,RFC3492]]   [RFC3492] :: Costello, A., "Punycode: A Bootstring encoding of Unicode
              for Internationalized Domain Names in Applications
              (IDNA)", RFC 3492, March 2003.

[[ref_RFC3927,RFC3927]]   [RFC3927] :: Cheshire, S., Aboba, B., and E. Guttman, "Dynamic
              Configuration of IPv4 Link-Local Addresses", RFC 3927, May
              2005.

[[ref_RFC4033,RFC4033]]   [RFC4033] :: Arends, R., Austein, R., Larson, M., Massey, D., and S.
              Rose, "DNS Security Introduction and Requirements", RFC
              4033, March 2005.

[[ref_RFC4291,RFC4291]]   [RFC4291] :: Hinden, R. and S. Deering, "IP Version 6 Addressing
              Architecture", RFC 4291, February 2006.

[[ref_RFC4795,RFC4795]]   [RFC4795] :: Aboba, B., Thaler, D., and L. Esibov, "Link-local
              Multicast Name Resolution (LLMNR)", RFC 4795, January
              2007.

[[ref_RFC4861,RFC4861]]   [RFC4861] :: Narten, T., Nordmark, E., Simpson, W., and H. Soliman,
              "Neighbor Discovery for IP version 6 (IPv6)", RFC 4861,
              September 2007.

[[ref_RFC4862,RFC4862]]   [RFC4862] :: Thomson, S., Narten, T., and T. Jinmei, "IPv6 Stateless
              Address Autoconfiguration", RFC 4862, September 2007.

[[ref_RFC5226,RFC5226]]   [RFC5226] :: Narten, T. and H. Alvestrand, "Guidelines for Writing an
              IANA Considerations Section in RFCs", BCP 26, RFC 5226,
              May 2008.

[[ref_RFC5890,RFC5890]]   [RFC5890] :: Klensin, J., "Internationalized Domain Names for
              Applications (IDNA): Definitions and Document Framework",
              RFC 5890, August 2010.

[[ref_RFC6281,RFC6281]]   [RFC6281] :: Cheshire, S., Zhu, Z., Wakikawa, R., and L. Zhang,
              "Understanding Apple's Back to My Mac (BTMM) Service", RFC
              6281, June 2011.

[[ref_RFC6760,RFC6760]]   [RFC6760] :: Cheshire, S. and M. Krochmal, "Requirements for a Protocol
              to Replace the AppleTalk Name Binding Protocol (NBP)", RFC
              6760, February 2013.

[[ref_RFC6763,RFC6763]]   [RFC6763] :: Cheshire, S. and M. Krochmal, "DNS-Based Service
              Discovery", RFC 6763, February 2013.

[[ref_Zeroconf,Zeroconf]]   [Zeroconf] :: Cheshire, S. and D. Steinberg, "Zero Configuration
...
[appendix]
== Design Rationale for Choice of UDP Port Number
...
[appendix]
== Design Rationale for Not Using Hashed Multicast Addresses
...
example, IPv6 Neighbor Discovery [<<ref_RFC4861>>] sends Neighbor
...
"Domain Names - Implementation and Specification" [<<ref_RFC1035>>] says:
...
* It is common in software engineering to have size limits that are a
...
* It is common in software engineering for the semantic "length" of
...
* The text describing the total length of a domain name mentions
...
Finally, "Clarifications to the DNS Specification" [<<ref_RFC2181>>] offers
...
Specification" [<<ref_RFC2181>>] statement that "." is the "zero length full
...
[appendix]
== Benefits of Multicast Responses
...
[appendix]
== Design Rationale for Encoding Negative Responses
...
"Punycode" [<<ref_RFC3492>>].  Punycode is a remarkably ingenious encoding
...
Compatible Encodings" [<<ref_RFC5890>>] proposed for Unicast DNS may be used
...
for Microsoft Windows [<<ref_B4W>>], Linux, and other platforms.
...
AppleTalk Name Binding Protocol [<<ref_RFC6760>>] over IP.  As a result of
...
224.0.0.251 with IANA [<<ref_MC4>>] and began writing code to test and
...
* "Requirements for a Protocol to Replace the AppleTalk Name Binding
  Protocol (NBP)" [<<ref_RFC6760>>] is an overview explaining the AppleTalk
...
* "Discovering Named Instances of Abstract Services using DNS" [<<ref_NIAS>>]
  proposed a way to perform NBP-like service discovery using DNS-
  compatible names and record types.

* "Multicast DNS" (this document) specifies a way to transport those
...
publicly accessible mDNSResponder CVS source code repository) [<<ref_BJ>>].
...
Multicast Name Resolution (LLMNR)" [<<ref_RFC4795>>], which is substantially
...
for a protocol to replace AppleTalk NBP [<<ref_RFC6760>>].
...
works using Unicast DNS servers, using DNS Update [<<ref_RFC2136>>] [<<ref_RFC3007>>]
...
class in Android 4.1 "Jelly Bean" (API Level 16) [<<ref_NSD>>].
...
====
Errata ID: https://www.rfc-editor.org/errata/eid4977[4977] +
Status: Reported +
Type: Technical +
...
Section <<_negative_responses>> says:

[subs=+macros]
....
The 'Next Domain Name' field contains the record's own name. pass:quotes[*When
