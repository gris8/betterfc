...
[frame=none, cols="^a", width="99%"]
|===
|          [big]#DOMAIN NAMES - CONCEPTS AND FACILITIES#
|===
...
- Host name to address mappings were maintained by the Network
  ...
  was FTPed by all hosts [<<ref_RFC-952>>, <<ref_RFC-953>>].  The total network
...
[<<ref_IEN-116>>, <<ref_RFC-799>>, <<ref_RFC-819>>, <<ref_RFC-830>>].  The proposals varied, but a
...
was described in [<<ref_RFC-882>>, <<ref_RFC-883>>].  Based on experience with several
...
This is particularly true in mail addressing [<<ref_Quarterman_86>>].
...
- Where [.rfc-error-text]#there tradeoffs# between the cost of acquiring data, the
...
to avoid many of [.rfc-error-text]#the the# complicated problems found in general purpose
database systems.
...
administrator [<<ref_RFC-1033>>].  For a name server, this configuration data
...
might share a database consisting of [.rfc-error-text]#the the# zones managed by the name
...
from network or host numbers to names; NetBIOS domains [<<ref_RFC-1001>>, <<ref_RFC-1002>>]
are flat because that is appropriate for that application.
...
decisions about the top levels of the tree originated in <<ref_RFC-920>>.
Current policy for the top levels is discussed in [<<ref_RFC-1032>>].  MILNET
conversion issues are covered in [<<ref_RFC-1031>>].
...
converting <local-part> into a single label ([.rfc-error-text]#regardles# of dots it
...
design also must take into account the scheme for mail exchanges [<<ref_RFC-974>>].
...
MX     ::       identifies a mail exchange for the
                domain.  See [.rfc-error-text]#[<<ref_RFC-974>># for details.
...
[options="autowidth",cols="<,^,^,>", stripes=none, frame=all]
|===
|ISI.EDU.       | MX   |     10 | VENERA.ISI.EDU.
|               | MX   |     10 | VAXA.ISI.EDU.
|VENERA.ISI.EDU.| A  2+|     128.9.0.32
|               | A  2+|     10.1.0.52
|VAXA.ISI.EDU.  | A  2+|     10.2.0.27
|               | A  2+|     128.9.0.33
|===
...
For example, suppose a name server was processing a query [.rfc-error-text]#with for#
USC-ISIC.ARPA, asking for type A information, and had the following resource
records:
...
[.rfc-error-text]#For example, a mailer tying to send mail to Mockapetris@ISI.EDU might
...
The optional completion services described in RFCs <<ref_RFC-882>> and <<ref_RFC-883>> have been
...
there are some administrative groupings discussed in [<<ref_RFC-1032>>] which
...
dedicated to individual departments or schools.  [<<ref_RFC-1033>>] catalogs
available DNS software [.rfc-error-text]#an# discusses administration procedures.
...
- Queries contain a bit called recursion desired or RD.  This
  bit [.rfc-error-text]#specifies specifies# whether the requester wants recursive
...
c. If at some label, a match is impossible (i.e., the
   ...
   the "{snglstar_}" label exists.
+
If the "{snglstar_}" label does not exist, check whether the name
we are looking for is the original QNAME in the query
or a name we have followed due to a CNAME.  If the name
is original, set an authoritative name error in the
response and exit.  Otherwise just exit.
+
If the "{snglstar_}" label does exist, match RRs at that node
against QTYPE.  If any match, copy them into the answer
section, but set the owner of the RR to be QNAME, and
not the node with the "{snglstar_}" label.  Go to step 6.
====
...
6. Using local data only, attempt to add other RRs which may be
   useful to the additional section of the [.rfc-error-text]#query#.  Exit.
...
the form "{snglstar_}.<anydomain>", where <anydomain> is any domain name.
...
to look at this is that the "{snglstar_}" label always matches at least one whole
...
- When the query name or a name between the wildcard domain and
  the query name is [.rfc-error-text]#know# to exist.  For example, if a wildcard
  RR has an owner name of "{snglstar_}.X", and the zone also contains RRs
...
naming shorthands that use search lists [.rfc-error-text]#beacuse# a popular shorthand,
...
assume that its copy of the zone is obsolete [.rfc-error-text]#an# discard it.
...
// TODO(andy): I should probably use a table.
....
...
Header     | OPCODE=SQUERY, RESPONSE, AA                       |
           +---------------------------------------------------+
Question   | QNAME=USC-ISIC.ARPA., QCLASS=IN, pass:quotes[*QTYPE=A*]          |
           +---------------------------------------------------+
Answer     | USC-ISIC.ARPA. 86400 IN CNAME      C.ISI.EDU.     |
           +---------------------------------------------------+
Authority  | <empty>                                           |
...
[[ref_IEN-116,IEN-116]] [IEN-116] ::      J. Postel, "Internet Name Server", IEN-116,
                USC/Information Sciences Institute, August 1979.
+
A name service obsoleted by the Domain Name System, but
still in use.

[[ref_Quarterman_86,Quarterman 86]][Quarterman{nbsp}86] ::  Quarterman, J., and J. Hoskins, "Notable Computer
...
[[ref_RFC-799,RFC-799]] [RFC-799] ::      D. Mills, "Internet Name Domains", RFC-799, COMSAT,
                September 1981.
+
...
+
Obsolete.  See <<ref_RFC-952>>.

[RFC-811] ::      K. Harrenstien, V. White, and E. Feinler, "Hostnames
                Server", RFC-811, Network Information Center, SRI
                International, March 1982.
+
Obsolete.  See <<ref_RFC-953>>.

[RFC-812] ::      K. Harrenstien, and V. White, "NICNAME/WHOIS", RFC-812,
                Network Information Center, SRI International, March
                1982.

[[ref_RFC-819,RFC-819]] [RFC-819] ::      Z. Su, and J. Postel, "The Domain Naming Convention for
...
[[ref_RFC-830,RFC-830]] [RFC-830] ::      Z. Su, "A Distributed System for Internet Name Service",
...
[[ref_RFC-882,RFC-882]] [RFC-882] ::      P. Mockapetris, "Domain names - Concepts and
                Facilities," RFC-882, USC/Information Sciences
                Institute, November 1983.
+
Superceeded by this memo.

[[ref_RFC-883,RFC-883]] [RFC-883]  ::     P. Mockapetris, "Domain names - Implementation and
                Specification," RFC-883, USC/Information Sciences
                Institute, November 1983.
+
Superceeded by this memo.

[[ref_RFC-920,RFC-920]] [RFC-920] ::      J. Postel and J. Reynolds, "Domain Requirements",
                RFC-920, USC/Information Sciences Institute
                October 1984.
+
Explains the naming scheme for top level domains.

[[ref_RFC-952,RFC-952]] [RFC-952] ::      K. Harrenstien, M. Stahl, E. Feinler, "DoD Internet Host
                Table Specification", RFC-952, SRI, October 1985.
+
Specifies the format of HOSTS.TXT, the host/address
table replaced by the DNS.

[[ref_RFC-953,RFC-953]] [RFC-953] ::      K. Harrenstien, M. Stahl, E. Feinler, "HOSTNAME Server",
...
+
Describes changes to <<ref_RFC-882>> and <<ref_RFC-883>> and reasons for
them.  Now obsolete.

[[ref_RFC-974,RFC-974]] [RFC-974] ::      C. Partridge, "Mail routing and the domain system",
...
[[ref_RFC-1001,RFC-1001]] [RFC-1001] ::     NetBIOS Working Group, "Protocol standard for a NetBIOS
...
[[ref_RFC-1002,RFC-1002]] [RFC-1002] ::     NetBIOS Working Group, "Protocol standard for a NetBIOS
                service on a TCP/UDP transport: Detailed
                Specifications", RFC-1002, March 1987.

[[ref_RFC-1010,RFC-1010]] [RFC-1010] ::     J. Reynolds and J. Postel, "Assigned Numbers", RFC-1010,
                USC/Information Sciences Institute, May 1987
+
Contains socket numbers and mnemonics for host names,
operating systems, etc.

[[ref_RFC-1031,RFC-1031]] [RFC-1031] ::     W. Lazear, "MILNET Name Domain Transition", RFC-1031,
                November 1987.
+
Describes a plan for converting the MILNET to the DNS.

[[ref_RFC-1032,RFC-1032]] [RFC-1032] ::     M. K. Stahl, "Establishing a Domain - Guidelines for
                Administrators", RFC-1032, November 1987.
+
Describes the registration policies used by the NIC to
administer the top level domains and delegate subzones.

[[ref_RFC-1033,RFC-1033]] [RFC-1033] ::     M. K. Lottor, "Domain Administrators Operations Guide",
                RFC-1033, November 1987.
+
A cookbook for domain administrators.

[Solomon{nbsp}82] ::   M. Solomon, L. Landweber, and D. Neuhengen, "The CSNET
...
====
Errata ID: https://www.rfc-editor.org/errata/eid564[564] +
Status: Verified +
Type: Technical +
...
The usual mail address <local-part>@<mail-domain> is mapped into a
domain name by converting <local-part> into a single label
(regardlespass:quotes[*s*] of dots it contains), converting <mail-domain> into a
domain name using the usual text format for domain names (dots
denote label breaks), and concatenating the two to form a single
domain name.
....
...
==== 4740
Errata ID: https://www.rfc-editor.org/errata/eid4740[4740] +
Status: Verified +
Type: Technical +
Publication Format(s) : TEXT +
...
<<errata_4740,Section 4.3.2>> says:
...
....
6. Using local data only, attempt to add other RRs which may be
   useful to the additional section of the pass:quotes[*response*].  Exit.
....

NOTE: Changed "query" to "response".

<<errata_4740,Section 4.3.2>> describes the algorithm used by nameservers to answer queries. I.e.,
a nameserver receives a query from a client, and then it performs this algorithm
in order to construct a response to send to the client.

Steps 1, 3, and 5 of this algorithm talk about setting fields or bits in the response,
or about adding resource records to the answer section of the response,
but then step 6 says to add resource records to the additional section of the *query*.
This doesn't make any sense, because the server is constructing a response to a query that
it's already received. "Response" must have been intended instead.

==== 565

Errata ID: https://www.rfc-editor.org/errata/eid565[565] +
Status: Verified +
Type: Editorial +
Publication Format(s) : TEXT +
...
[subs=+macros]
....
This bit pass:quotes[*specifies*] whether the requester wants recursive service for this
query. Clients may.....
....
...
Errata ID: https://www.rfc-editor.org/errata/eid1074[1074] +
Status: Verified +
Type: Editorial +
Publication Format(s) : TEXT +
...
[subs=+macros]
....
  - Where there pass:quotes[*are*] tradeoffs between the cost of acquiring data, the
....

<<errata_1074b,Section 3.6.2.>> says:
...
[subs=+macros]
....
  For example, suppose a name server was processing a query pass:quotes[*for*]
....

<<errata_1074c,Section 4.3.4.>> misspells 'because':

[subs=+macros]
....
  This feature can be particularly important in a system which
  implements naming short shorts that use search lists pass:quotes[*beacuse*]
....
...
Errata ID: https://www.rfc-editor.org/errata/eid4943[4943] +
Status: Verified +
Type: Editorial +
Publication Format(s) : TEXT +
...
<<errata_4943,Section 4.3.5.>> says:

[subs=+macros]
....
it must assume that its copy of the zone is obsolete pass:quotes[*an*] discard it.
....
...
[subs=+macros]
....
it must assume that its copy of the zone is obsolete pass:quotes[*and*] discard it.
....

NOTE: Fix typo; "an" should be "and".

=== Held for Document Update

==== 755

Errata ID: https://www.rfc-editor.org/errata/eid755[755] +
...
Section <<sec_6.2.8,6.2.8>>. in the diagram shows
...
Errata ID: https://www.rfc-editor.org/errata/eid5316[5316] +
Status: Held for Document Update +
Type: Technical +
Publication Format(s) : TEXT +
...
used to test for wildcards in an authoritative zone; such a query is the
only way to get a response containing RRs with an owner name with * in
it.  The result of such a query should not be pass:quotes[*cached*].
....
...
used to test for wildcards in an authoritative zone; such a query is the
only way to get a response containing RRs with an owner name with * in
it.  The result of such a query should not be pass:quotes[*used to synthesize RRs*].
....
...
Errata ID: https://www.rfc-editor.org/errata/eid4270[4270]
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
...
Section <<sec_2.4,2.4>> says:

[subs=+macros]
....
In the interests of performance, implementations may couple these
functions.  For example, a resolver on the same machine as a name server
might share a database consisting of pass:quotes[*the the*] zones managed by the name
server and the cache managed by the resolver.
....
...
In the interests of performance, implementations may couple these
functions.  For example, a resolver on the same machine as a name server
might share a database consisting of pass:quotes[*the*] zones managed by the name
server and the cache managed by the resolver.
....

NOTE: Just a duplicate "the".


==== 4269

Errata ID: https://www.rfc-editor.org/errata/eid4269[4269] +
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
Reported By: Jean-Philippe Paradis +
...
Section <<sec_2.3,2.3>> says:

[subs=+macros]
....
The organization of the domain system derives from some assumptions
about the needs and usage patterns of its user community and is designed
to avoid many of pass:quotes[*the the*] complicated problems found in general purpose
database systems.
....
...
The organization of the domain system derives from some assumptions
about the needs and usage patterns of its user community and is designed
to avoid many of pass:quotes[*the*] complicated problems found in general purpose
database systems.
....
...
Errata ID: https://www.rfc-editor.org/errata/eid4229[4229] +
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
...
Section <<sec_4.2.2,4.2.2>> says:

[subs=+macros]
....
[RFC-1033] catalogs available DNS software pass:quotes[*an*] discusses administration
procedures.
....
...
[subs=+macros]
....
[RFC-1033] catalogs available DNS software pass:quotes[*and*] discusses administration
procedures.
....
...
Errata ID: https://www.rfc-editor.org/errata/eid4230[4230] +
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
Reported By: Sun Congyou +
...
Section <<sec_4.3.3,4.3.3>> says:

[subs=+macros]
....
- When the query name or a name between the wildcard domain and
  the query name is pass:quotes[*know*] to exist.
....
...
....
- When the query name or a name between the wildcard domain and
  the query name is pass:quotes[*known*] to exist.
....


==== 4511

Errata ID: https://www.rfc-editor.org/errata/eid4511[4511] +
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
...
Section <<sec_3.6,3.6>> says:


[subs=+macros]
....
identifies a mail exchange for the domain. See pass:quotes[*[RFC-974*] for details.
....
...
NOTE:  Just missing the "]"


==== 4893

Errata ID: https://www.rfc-editor.org/errata/eid4893[4893] +
Status: Held for Document Update +
Type: Editorial +
Publication Format(s) : TEXT +
