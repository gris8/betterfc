...
|===
|      [big]#Selection and Operation of Secondary DNS Servers#
|===
...
[horizontal]
DNS               ::   The Domain Name System [<<ref_RFC1034>>, <<ref_RFC1035>>].
...
* While positive DNS results are usually cached, the lack of a
 result [.rfc-error-text]#is not cached#.  Thus, unnecessary inability to resolve
...
// sec.4.1
=== Servers behind intermittent connections
...
[<<ref_RFC1631>>] exists between a resolver and server.  Despite what
[<<ref_RFC1631>>] suggests, NATs in practice do not translate addresses
embedded in packets, only those in the headers.  As [<<ref_RFC1631>>]
...
form a Resource Record Set [<<ref_RFC2181>>], all must be returned (or none),
...
// sec.6
== Finding Suitable Secondary Servers
...
[<<ref_RFC1982>>] for a more rigorous definition of the serial number.
...
Do not simply change the value from 1000 to 11.  A secondary
...
the current value, but less that 2{caret}31 bigger (2{caret}31 is 2147483648).
This is then an increment of the serial number [<<ref_RFC1982>>].
...
is thus another increment (the value added is less than 2{caret}31).
...
2{caret}32 (4294967296), so 11 is identical to 4294967307 (4294967296 +
 11).  4294967307 is just 294967307 greater than 4000000000, and
294967307 is well under 2{caret}31, this is therefore an increment.
...
[[ref_RFC1631,RFC1631]]   [RFC1631] ::  Egevang, K., Francis, P., "The IP Network Address Translator
               (NAT)", RFC 1631, May 1994

[[ref_RFC1982,RFC1982]]   [RFC1982] ::  Elz, R., Bush, R., "Serial Number Arithmetic",
               RFC 1982, August 1996.

[[ref_RFC2181,RFC2181]]   [RFC2181] ::  Elz, R., Bush, R., "Clarifications to the DNS specification",
               RFC 2181, July 1997.
...
=====
Errata ID: https://www.rfc-editor.org/errata/eid4631[4631] +
...
[NOTE]
====
BCP 16 predates RFC 2308 (Negative Caching of DNS Queries),
and the blanket statement that "lack of a result is not cached"
is inaccurate by modern standards.
