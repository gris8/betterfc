...
|===
|               [big]#DNS-Based Service Discovery#
|===
...
described using a DNS SRV [<<ref_RFC2782>>] and DNS TXT [<<ref_RFC1035>>] record.
...
pairs, described in Section <<sec_6>>.  A client discovers the list of
available instances of a given service type using a query for a DNS
PTR [<<ref_RFC1035>>] record with a name of the form "<Service>.<Domain>",
which returns a set of zero or more names, which are the names of the
aforementioned DNS SRV/TXT record pairs.

This specification is compatible with both Multicast DNS [<<ref_RFC6762>>]
...
it with the DNS Update [<<ref_RFC2136>>] [<<ref_RFC3007>>] keys to give it permission
...
(see Section <<sec_11>>, "Discovery of Browsing and Registration Domains"),
...
almost any kind of data, for almost any purpose" [<<ref_RFC2181>>].  By
...
====
// XXX(andy): the original RFC uses parentheses around the list number: (i)

["lowerroman"]
. The ability to query for services of a certain type in a
certain logical domain, and receive in response a list of named
instances (network browsing or "Service Instance Enumeration").

. Given a particular named instance, the ability to efficiently
...
. Instance names should be relatively persistent.  If a user
...
the AppleTalk Name Binding Protocol (NBP)" [<<ref_RFC6760>>].  That document
...
Traditional DNS SRV records [<<ref_RFC2782>>] are useful for locating
...
Printing Protocol (IPP) [<<ref_RFC2910>>] printers at Example Co., then
...
another in the DNS namespace) [<<ref_RFC1035>>].
...
friendly name consisting of arbitrary Net-Unicode text [<<ref_RFC5198>>].  It
MUST NOT contain ASCII control characters (byte values 0x00-0x1F and
0x7F) [<<ref_RFC20>>] but otherwise is allowed to contain any characters,
...
UTF-8 [<<ref_RFC3629>>] "Net-Unicode" (Unicode Normalization Form C)
[<<ref_RFC5198>>] text.  For further discussion of text encodings, see
...
records [<<ref_RFC2782>>].  The first label of the pair is an underscore
character followed by the Service Name [<<ref_RFC6335>>].  The Service Name
...
details, see Section <<sec_7>>, "Service Names".
...
"local.", meaning "link-local Multicast DNS" [<<ref_RFC6762>>], or it may be
...
for host names [<<ref_RFC1033>>] [<<ref_RFC1034>>] [<<ref_RFC1035>>], and rich-text service
...
straightforward canonical precomposed UTF-8 [<<ref_RFC3629>>] "Net-Unicode"
(Unicode Normalization Form C) [<<ref_RFC5198>>] text.  In cases where the
...
algorithm [<<ref_RFC3492>>] to convert the UTF-8 name to an IDNA "A-label"
[<<ref_RFC5890>>], beginning with the top-level label, then issuing the query
...
a different domain.  Similarly, when using subtypes (see Section <<sec_7.1>>,
...
Section <<sec_6>>, "Data Syntax for DNS-SD TXT Records".
...
application protocol [<<ref_RFC6335>>].  Some protocols like the X Window
...
// sec.6
[[sec_6]]
==  Data Syntax for DNS-SD TXT Records
...
(port 515) protocol [<<ref_RFC1179>>] often specifies a queue name [BJP].
...
datatype [<<ref_RFC2782>>] may still be used in other contexts without any
...
Note that when using Multicast DNS [<<ref_RFC6762>>] the maximum packet size
...
bytes, as described below in Section <<sec_6.2>>.
...
the DNS specification [<<ref_RFC1035>>] and are not specific to DNS-SD.
...
An empty TXT record containing zero strings is not allowed [<<ref_RFC1035>>].
...
single 512-byte DNS message [<<ref_RFC1035>>].
...
Multicast DNS [<<ref_RFC6762>>] offload, so that computers can sleep and
...
<<sec_6.4>>).  Everything after the first '=' character to the end of the
string (including subsequent '=' characters, if any) is the value
(Section <<sec_6.5>>).  No quotation marks are required around the value,
...
[<<ref_RFC1179>>], the LPR protocol provides no way for the client to
...
[<<ref_RFC6762>>] this is important because multicast traffic is especially
...
size limit [<<ref_RFC1035>>].  Also, each constituent string of a DNS TXT
...
attribute will be US-ASCII [<<ref_RFC20>>] or UTF-8 [<<ref_RFC3629>>] text, but it is
...
representation, Base-64 [<<ref_RFC4648>>], or Unix-to-Unix (UU) encoding,
...
number in US-ASCII [<<ref_RFC20>>] text (e.g., "txtvers=1" or "txtvers=8"),
...
incompatible changes become unavoidable [<<ref_RFC6709>>].  Clients SHOULD
...
// sec.6.8.
===  Service Instances with Multiple TXT Records
...
the Service Name [<<ref_RFC6335>>].  The Service Name identifies what the
...
Transmission Protocol (SCTP) [<<ref_RFC4960>>], Datagram Congestion Control
Protocol (DCCP) [<<ref_RFC4340>>], Adobe's Real Time Media Flow Protocol
...
datatype [<<ref_RFC2782>>]; they are free to specify their own naming
conventions as appropriate.

The rules for Service Names [<<ref_RFC6335>>] state that they may be no more
...
data values may be UTF-8 [<<ref_RFC3629>>] representations of text, or even
(as in the example above) plain ASCII [<<ref_RFC20>>], but they do not have
...
<<sec_7.1>>, "Selective Instance Enumeration").
...
printing world has the lineprinter (LPR) protocol [<<ref_RFC1179>>] and the
Internet Printing Protocol (IPP) [<<ref_RFC2910>>], both of which cause
...
Note: When used with Multicast DNS [<<ref_RFC6762>>], the target host field
...
zone file or use DNS Update [<<ref_RFC2136>>] [<<ref_RFC3007>>].
...
IP printers could use Dynamic DNS Update [<<ref_RFC2136>>] [<<ref_RFC3007>>] to
...
their own PTR, SRV, and TXT names ending with ".local." [<<ref_RFC6762>>].
...
"Domain" option (option code 15) [<<ref_RFC2132>>], the DHCP "Domain Search"
option (option code 119) [<<ref_RFC3397>>], or IPv6 Router Advertisement
Options [<<ref_RFC6106>>].
...
IPv4 link-local addresses [<<ref_RFC3927>>] or IPv6 link-local addresses
[<<ref_RFC4862>>].
...
// sec.13,1
===  What web pages are being advertised from dns-sd.org?
...
// sec.13.2
===  What printer-configuration web pages are there?
...
// sec.13.3
===  How do I access the web page called "Service Discovery"?
...
For DNS queries, DNS Security Extensions (DNSSEC) [<<ref_RFC4033>>] should be
used where the authenticity of information is important.

For DNS updates, secure updates [<<ref_RFC2136>>] [<<ref_RFC3007>>] should generally
...
IANA manages the namespace of unique Service Names [<<ref_RFC6335>>].
...
behavior described in Section <<sec_8>>, then the registrant should request
...
[horizontal]
[[ref_AFP,AFP]]   [AFP] ::
               Mac OS X Developer Library, "Apple Filing Protocol
               Programming Guide", <http://developer.apple.com/
               documentation/Networking/Conceptual/AFP/>.

[[ref_BJ,BJ]]   [BJ] ::
               Apple Bonjour Open Source Software,
...
[[ref_SOAP,SOAP]]   [SOAP] ::
               Mitra, N., "SOAP Version 1.2 Part 0: Primer", W3C
               Proposed Recommendation 24 June 2003,
               <http://www.w3.org/TR/2003/REC-soap12-part0-20030624>.

[[ref_Unicode6,Unicode6]]   [Unicode6] ::
...
Multicast DNS [<<ref_RFC6762>>] provides zero-configuration ad hoc service
...
already has a dynamic registration protocol [<<ref_RFC2136>>] [<<ref_RFC3007>>].
...
DNS already has existing mechanisms for security [<<ref_RFC4033>>].
...
// appendix.b
[appendix]
==  Ordering of Service Instance Name Components
...
below in Sections <<sec_B.1>>, <<sec_B.2>>, and <<sec_B.3>>.
...
Dynamic Update [<<ref_RFC2136>>] [<<ref_RFC3007>>] for printers registering in the
...
// appendix.d
[appendix]
==  Choice of Factory-Default Names

When a DNS-SD service is advertised using Multicast DNS [<<ref_RFC6762>>], if
...
described in the Multicast DNS specification [<<ref_RFC6762>>], upon
...
// appendix.e
[appendix]
==  Name Encodings in the Domain Name System

Although the original DNS specifications [<<ref_RFC1033>>] [<<ref_RFC1034>>]
[<<ref_RFC1035>>] recommend that host names contain only letters, digits, and
...
Name. "Clarifications to the DNS Specification" [<<ref_RFC2181>>] directly
...
// appendix.f
[appendix]
==  "Continuous Live Update" Browsing Model
...
* "Multicast DNS" [<<ref_RFC6762>>] specifies a way to transport those DNS-
